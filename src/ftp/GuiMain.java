...
public class GuiMain extends JFrame implements ActionListener,MouseListener{
	...
	private JButton btnConnect,btnDisconnect,btnDownload,btnUpload, btnDeleteFile, btnAddFolde,btnRefresh,btnRename,btnBack;
	...
	public void giaoDien(){
		...
		/*--------------------------------Status------------------------------------*/
		jSc =  new JScrollPane();
		jSc.setBounds(5, 60, 700, 135);
		jSc.setBackground(Color.WHITE);
		jSc.setBorder(javax.swing.BorderFactory.createTitledBorder("SERVER PHẢN HỒI:"));
		jSc.setViewportView(statusConnect);
		statusConnect.setEditable(false);
		statusConnect.setBackground(new Color(36,47,65));
		statusConnect.setForeground(new Color(127, 100, 138));
		...
		/*-----------------------------------------------------------------------------------------------*/
		JLabel lbTieuDe = new JLabel("ĐỒ ÁN LẬP TRÌNH MẠNG");
		...
		delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				int n= JOptionPane.showConfirmDialog(delete,"Bạn có chắc cnắn muốn xóa file này ?","Xóa file",JOptionPane.YES_NO_OPTION);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btnDeleteFile){
			int n= JOptionPane.showConfirmDialog(btnDeleteFile,"Bạn có chắc cnắn muốn xóa file này ?","Xóa file",JOptionPane.YES_NO_OPTION);
			deleteFileOrFolder(n);
		}
		/* --------------------------- New Folder ------------------------------*/
	...
	public void getLink(int i){
		...
		if(!model.getValueAt(selectRowIndex,3).toString().equals("")){
			...
			link = link.replace("//", "/");
			// If i == 1 thì với cập nhập lại bảng tức là Double click thì với cập nhập lại table. Tuc la click vao thuc muc
			if(i==1){
				path.setText(link);
				link = path.getText();
				deleteAllRow();
		...
		// Nếu cột thứ 3 có kích thước size tức là thư mục kiemTràolder = false thì;
		// Truyền luôn đường dẫn và kích thước file cho lớp Dow
		dow.setSize(getSizeFile);
		dow.setURL(link);
		
		// Truyền luôn đường dẫn và kích thước file cho lớp Up
		// Mac dinh cu gui path.getText(). Neu rightClick vao Thu muc vs gui link...(Tuc la gui link cua thu muc)
